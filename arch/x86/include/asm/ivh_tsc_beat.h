 ...
 *   3. struct ivh_cs_beat (the CS-preemption-stamp predicate) and the raw
 *      TSC<->ns conversion helpers used only by Part C
 *      (ivh_vact_capacity). BOTH EXCLUDED: sec 1.7's artifact list bundles
 *      CS-stamp + holder-identity together as "fully wired, large, but
 *      default-OFF, never enabled in production, predicate has a measured
 *      hard ceiling of 78.57% sensitivity", and Part C is separately listed
 *      there as "attempted, measured regression, root-caused, not shipped".
 *      Neither is reachable from anything this step ports.
 *
 * Do not "restore" sections 2/3 above without re-reading sec 1.7 first.
 */
